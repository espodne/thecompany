...
export const DesktopLayout = () => {
  ...
  const { activeSection, containerRef, isInitialized, forceUpdate, setActiveSectionManually } = useActiveSection()
  const [hideIndicator, setHideIndicator] = useState(false)
  const screenHeight = useScreenHeight()
  const isSmallHeight = screenHeight < 800

  // Принудительная инициализация после загрузки проектов
  useEffect(() => {
        ...
        <div className="w-90 h-screen flex flex-col fixed left-0 top-0">
          ...
          <div className="mt-auto overflow-y-auto desktop-navigation">
            <Navigation
              ...
              projectsCount={0}
              hideIndicator={hideIndicator || !isInitialized}
              onNavigationClick={(sectionId) => {
                setHideIndicator(false)
